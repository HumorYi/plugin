...
let functionUtils = {
    ...
    math: {
      ...
      commonDivisor: {
        ...
        allForArrayNumber(arrNumber, type, max) {
          ...
          // 必须是数字数组
          if (/[^\d]/.test(arrNumber.join(''))) {
            throw TypeError('please transfer a number array, thanks!')
          }
          ...
          for (; j <= min; j++) {
            ...
            for (i = 0; i < uniqueArrNumberLen; i++) {
              ...
              if (uniqueArrNumber[i] % j !== 0) {
                break
              }

              // 不到最后一个通过就不是所有数的共同公约数
              if (i !== uniqueArrNumberLen - 1) {
                continue
              }
      ...
      addPrivateProperty(o, name, predicate) {
        ...
        o['set' + name] = val => {
          if (predicate && !predicate(val).result) {
            throw new Error(
              'set' + name + ': invaild, because ' + name[0].toLowerCase() + name.slice(1) + ' ' + predicate(val).msg
            )
          }
  ...
  format: {
    ...
    date(oDate, formatStr) {
      ...
      return formatStr
        .replace(/yyyy|YYYY/, oDate.getFullYear())
        ...
        .replace(/dd|DD/, date > 9 ? date.toString() : '0' + date)
        .replace(/d|D/g, date)
  ...
  verify: {
    ...
    isInteger(data) {
      return (
        typeof data === 'number' && isFinite(data) && data > 0 && data === Math.abs(data) && data === Math.ceil(data)
      )
    },
    /**
     * @desc 是否邮箱
  ...
  getDeviceBrowserType() {
    ...
    if (sUserAgent.search(/MicroMessenger/gi) !== -1) {
      return 'weixin'
